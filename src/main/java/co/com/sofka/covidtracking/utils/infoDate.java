...
public class infoDate {
    ...
    public static DateModel generateDate(){
        ...
        dates.setYearinit(String.valueOf(fake.number().numberBetween(2020,2021)));
       if(dates.getYearinit().equals(String.valueOf(2020))){
            dates.setMonthinit(String.format("%02d",fake.number().numberBetween(2,12)));
            if(dates.getMonthinit().equals(String.format("%02d",2))){
                dates.setDayinit(String.format("%02d",fake.number().numberBetween(1,28)));
            }else{
                dates.setDayinit(String.format("%02d",fake.number().numberBetween(1,30)));
            }
        }else {
            dates.setMonthinit(String.format("%02d",fake.number().numberBetween(1,3)));
            if(dates.getMonthinit().equals(String.format("%02d",3))){
                dates.setDayinit(String.format("%02d",fake.number().numberBetween(1,7)));
            }if(dates.getMonthinit().equals(String.format("%02d",2))){
                dates.setDayinit(String.format("%02d",fake.number().numberBetween(1,28)));
            }else {
                dates.setDayinit(String.format("%02d",fake.number().numberBetween(1,30)));
